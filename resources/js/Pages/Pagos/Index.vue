...
/** Marca un pago como 'pagado' vía API **/
const marcarComoPagado = async (pagoId) => {
  try {
    const confirmacion = await Swal.fire({
      title: '¿Confirmar Pago?',
      text: '¿Deseas marcar este pago como pagado?',
      icon: 'question',
    ...
    const { data } = await axios.post(`/pagos/${pagoId}/marcar-pagado`);
    if (data.success) {
      await Swal.fire('Éxito', 'El pago fue marcado como pagado.', 'success');
    ...
    Swal.fire('Error', 'No se pudo marcar el pago. Intenta nuevamente.', 'error');
  }
};
...
<td class="px-4 py-3 border text-center">
  <div class="flex justify-center space-x-2">
    ...
    >
      <svg
        v-if="cargandoQR && pagoSeleccionado?.id === pago.id"
    ...
    <button
      v-if="pago.estado === 'pendiente'"
      @click="marcarComoPagado(pago.id)"
      class="inline-flex items-center px-3 py-2 bg-green-600 text-white text-sm font-medium rounded-lg hover:bg-green-700 transition-colors shadow-sm"
    >
      <svg
        class="w-4 h-4 mr-2"
        fill="none" stroke="currentColor" viewBox="0 0 24 24"
