...
const apellido = ref(props.garante.apellido);
const ci = ref(props.garante.ci);
const domicilio = ref(props.garante.domicilio);
const nombre = ref(props.garante.nombre);
const telefono = ref(props.garante.telefono);

const enviar = () => {
  router.put(`/garantes/${props.garante.id}`, {
    ...
    onError: () => {
      Swal.fire({
        title: 'Error',
        text: 'Hubo un problema al actualizar el garante.',
        icon: 'error',
...
<template>
  <Head title="Editar Garante" />

  <AuthenticatedLayout>
    <template #header>
      <h2 class="text-xl font-semibold leading-tight text-gray-800">Editar Garante</h2>
    </template>
